...
def db_manager(db_path):
    config = DBConfig(
        db_path=db_path, passphrase="", encrypted=False, allow_unencrypted=True
    )
...
class TestThreadSafety:
    def test_concurrent_schema_setup(self, db_path):
        """Multiple threads can safely call setup_schema (idempotent)."""
        config = DBConfig(
            db_path=db_path, passphrase="", encrypted=False, allow_unencrypted=True
        )
        ...
        for t in threads:
            t.start()
        for t in threads:
            t.join(timeout=10)  # Increased timeout from default
