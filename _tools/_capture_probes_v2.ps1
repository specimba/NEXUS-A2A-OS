...
# 2. DENY (injection in evidence_notes)
$deny = $allow | ConvertTo-Json -Depth 8 | ConvertFrom-Json
$deny.request_id = "vid2-deny-" + $ts
$deny.case_id = "CASE-VID2-DENY-" + $ts
...
# 3. VERIFY FAIL (fresh id, link to allow eval)
$vf = Get-Content (Join-Path $samples "03-verification-failed.json") -Raw | ConvertFrom-Json
$vf.request_id = "vid2-vfail-" + $ts
$vf.case_id = "CASE-VID2-VERIFY-" + $ts
$vf.remediation_id = "rem-vid2-fail-" + $ts
...
$vp.request_id = "vid2-vpass-" + $ts
$vp.case_id = "CASE-VID2-VERIFY-" + $ts
$vp.remediation_id = "rem-vid2-pass-" + $ts
