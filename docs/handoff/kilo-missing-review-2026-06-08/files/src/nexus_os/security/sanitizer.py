...
class VerifiableOutput:
    ...
    def sign(self, signing_key: str = ""):
        ...
        self.signature = hmac.new(signing_key.encode('utf-8'), self.content_hash.encode('utf-8'), hashlib.sha256).hexdigest()

    def verify(self, signing_key: str = None) -> bool:
        """Verify content integrity.

        Recomputes the content hash and, if a signature is present,
        recomputes the HMAC over the content hash and compares it
        constant-time against the stored signature.
        """
        import hmac
        ...
        if key is None:
            # No key available to verify the signature against — fail closed.
            return False
        expected = hmac.new(key.encode('utf-8'), self.content_hash.encode('utf-8'), hashlib.sha256).hexdigest()
        return hmac.compare_digest(expected, self.signature)

    def verify_chain(self, other: 'VerifiableOutput', signing_key: str = None) -> bool:
        """Verify that two outputs form a valid chain."""
        if not self.verify(signing_key) or not other.verify(signing_key):
            return False
