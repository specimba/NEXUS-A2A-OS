...
- The Docker Compose source under `%USERPROFILE%\.docker\cagent\working_directories\docker-gordon-v7\...\default\docker-compose-kafka-bridge.yml` also contains raw Kafka credentials for the bridge/consumer/dashboard path. Do not copy those values into repo docs.
...
- `redis-nexus` is created from `%USERPROFILE%\.docker\cagent\working_directories\docker-gordon-v7\...\default\docker-compose-redis.yml`, where Redis is published as all-interface `6379:6379`.
...
- `6379` and `54322` are not exposed beyond localhost unless explicitly justified.
...
- Verified the active bridge compose source is `C:\Users\speci.000\.docker\cagent\working_directories\docker-gordon-v7\d9efce88-e41e-49fd-9b9d-6e79359e8bb4\default\docker-compose-kafka-bridge.yml`.
