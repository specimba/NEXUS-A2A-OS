...
class TrustKernel:
    ...
    def _derive_cdr_stage(
        ...
        previous_stage = self._coerce_cdr_stage(previous.cdr_stage)
        if trust < 0.15:
    ...
    @staticmethod
    def _coerce_cdr_stage(raw_stage: Any) -> CDRStage:
        if isinstance(raw_stage, CDRStage):
            return raw_stage
        try:
            return CDRStage(str(raw_stage))
        except ValueError:
            logger.warning("Unknown CDR stage %r; falling back to NORMAL", raw_stage)
            return CDRStage.NORMAL
