...
class TokenGuard:
    ...
    def __init__(
        ...
        # Token Policy Plane v2 reservation lifecycle.
    ...
    def can_spend(
        ...
        """Compatibility preflight for standalone/custom agents."""
    ...
    def reserve(
        ...
        """Preflight and reserve tokens without treating execution as complete."""
        budget_key = self._get_budget_key(agent_id)
        budget = self._budgets.get(budget_key)
    ...
    def commit_reservation(
        ...
        """Commit a reserved spend after successful execution."""
        with self._reservation_lock:
    ...
    def _scope_for_budget_key(self, budget_key: str) -> BudgetScope:
        """Map local budget keys into Token Policy Plane scopes."""
        try:
    ...
    # ── Deception Pattern Monitoring (Phase 1 Critical) ─────────

    def check_deception_patterns(
        self,
        agent_id: str,
        output: str,
        ...
        """
        Monitor agent output for deception and concealment patterns.
        ...
        Adds VAP-compliant audit entries for detected patterns.

        Args:
            agent_id: Agent to check.
            output: Agent output text to analyze.
            trace_id: Optional trace ID.

        Returns:
            Dict with monitoring results and CDR escalation status.
        """
        ...
        try:
            ...
            logger.warning("MisalignmentDetector not available, skipping deception check")
            return {"monitored": False, "reason": "detector_unavailable"}

        monitor_result = detector.monitor_output(agent_id, output, trace_id)

        # Log audit entries for any detected events (VAP-compliant)
        events = monitor_result.get("events", [])
        for event in events:
            self._log_audit(
                actor=agent_id,
                action=f"deception_detected:{event.get('pattern', 'unknown')}",
                input_tokens=0,
                output_tokens=len(output.encode()),
                context={
                    "detection_event": event,
                    "trace_id": trace_id,
                    "source": "token_guard_deception_monitor",
                },
            )

        # If CDR escalation recommended, add additional audit entry
        if monitor_result.get("cdr_escalate"):
            self._log_audit(
                actor=agent_id,
                action="deception_cdr_escalation_recommended",
                input_tokens=0,
                output_tokens=0,
                context={
                    "cdr_reason": monitor_result.get("cdr_reason"),
                    "risk_score": monitor_result.get("risk_score"),
                    "trace_id": trace_id,
                },
            )
            logger.critical(
                "TokenGuard deception monitor recommends CDR escalation for agent %s: %s",
                agent_id, monitor_result.get("cdr_reason")
            )
