...
class ArchivistFitter:
    ...
    def llm_synthesize_dossier(
        ...
        # Fallback: structured metadata synthesis. Tags come from the
        # records (was an empty set, which silenced the Key Findings
        # concept wikilinks whenever the LLM relay was unreachable).
        fallback_tags = {t for r in records for t in (r.topic_tags or [])}
        fallback = self._generate_dossier_markdown(topic, records, fallback_tags, [], citation_graph)
        return LLMSynthesisResult(
            success=False,
            content=fallback,
    ...
    def _generate_dossier_markdown(
        ...
        for i, r in enumerate(records[:10], 1):
            ...
            source_slug = self._slugify(title)
            self._ensure_source_stub(source_slug, r)
            lines.append(f"{i}. **[[sources/{source_slug}|{title}]]**")
        ...
        for tag in sorted(tags):
            ...
            self._ensure_concept_stub(tag_slug, tag)
            lines.append(f"- **[[concepts/{tag_slug}|{tag}]]**: {count} source(s) address this topic")
        lines.append("")
    ...
    def _ensure_concept_stub(self, slug: str, tag: str) -> None:
        """Idempotently create concepts/<slug>.md so [[concepts/...]] resolve."""
        try:
            path = self._wiki_root / "concepts" / f"{slug}.md"
            if path.exists():
                return
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(
                f"---\ntitle: {tag}\nadmission_class: concept\n---\n\n"
                f"# {tag}\n\nConcept stub — linked from dossiers. "
                f"See also: [[index]]\n",
                encoding="utf-8",
            )
        except OSError as e:
            logger.debug("Concept stub skipped for %s: %s", slug, e)

    def _ensure_source_stub(self, slug: str, record) -> None:
        """Idempotently create sources/<slug>.md so [[sources/...]] resolve."""
        try:
            path = self._wiki_root / "sources" / f"{slug}.md"
            if path.exists():
                return
            path.parent.mkdir(parents=True, exist_ok=True)
            ir = record.import_record
            title = ir.title or Path(ir.file_path).name
            arxiv = f"arxiv_id: {ir.arxiv_id}\n" if getattr(ir, "arxiv_id", None) else ""
            path.write_text(
            ...
            logger.debug("Source stub skipped for %s: %s", slug, e)

    def regenerate_index_moc(self) -> Path:
        """Rewrite wiki/index.md as a Map-of-Content with [[wikilinks]].

        Obsidian's graph view and the 7356 terminal UI both navigate from
        this page; regenerating keeps it in lockstep with what fit wrote.
        """
        root = self._wiki_root
        lines = [
            "---",
            "title: NEXUS Archive Index",
            "admission_class: index",
            "---",
            "",
            "# NEXUS Archive — Map of Content",
            "",
        ]
        for section in ("dossiers", "sources", "entities", "concepts"):
            section_dir = root / section
            if not section_dir.is_dir():
                continue
            entries = sorted(p.stem for p in section_dir.glob("*.md"))
            if not entries:
                continue
            lines.append(f"## {section.title()}")
            lines.append("")
            for stem in entries:
                label = stem.replace("dossier_", "").replace("_", " ").title()
                lines.append(f"- [[{section}/{stem}|{label}]]")
            lines.append("")
        path = root / "index.md"
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        logger.info("Regenerated wiki index MOC: %s", path)
        return path
    ...
    def fit_batch(self, dossier_candidates: Dict[str, List[CompiledRecord]]) -> List[Dossier]:
        ...
        if dossiers:
            try:
                self.regenerate_index_moc()
            except Exception as e:
                logger.warning("Index MOC regeneration failed: %s", e)
